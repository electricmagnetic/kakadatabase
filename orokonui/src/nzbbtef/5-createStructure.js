...
const defineParts = (tokens) => {
  const partSeparatorIndex = tokens.findIndex(
    (token) => token.type === "partSeparator"
  );
  ...
  const parts = [
    ...
    },
    {
      type: "tarsus",
      key: "tarsus",
      tokens: hasPartSeparator
        ? tokens.slice(partSeparatorIndex, tokens.length)
        : tokens,
    },
  ];
...
const defineLegs = (tokens) => {
  const legSeparatorIndex = tokens.findIndex(
    (token) => token.type === "legSeparator"
  );
    ...
    ? [
        {
          type: "left",
          key: "left",
          tokens: tokens.slice(0, legSeparatorIndex),
        },
        {
          type: "right",
          key: "right",
          tokens: tokens.slice(legSeparatorIndex, tokens.length),
        },
      ]
...
const createStructure = (tokens) => {
  ...
  return legs.map((leg) =>
    Object.assign({}, leg, { tokens: defineParts(leg.tokens) })
  );
};
