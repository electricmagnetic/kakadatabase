import {
  countInstancesInArray,
  getSeparatorsAsString,
  compileValidators,
} from "./helpers";
...
const validateLegSeparators = (tokens) =>
  countInstancesInArray(
    getSeparatorsAsString(tokens).split(" "),
    "legSeparator"
  ) <= 1;
...
const validateColours = (tokens) => {
  return tokens.reduce((isValid, token) => {
    const validators = Object.assign(
      {},
      token.tokens && {
        areChildrenValid: { validator: validateColours(token.tokens) },
      },
      token.isColourToken && {
        validColour: { validator: token.colour !== undefined },
      },
...
const validate = (tokens) => {
  const validators = {
    legSeparators: {
      label: "Leg separators valid?",
      validator: validateLegSeparators(tokens),
    },
    partSeparators: {
      label: "Part separators valid?",
      validator: validatePartSeparators(tokens),
    },
